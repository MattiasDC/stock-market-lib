...
class TestSignalSequence(unittest.TestCase):
    def setUp(self):
        ...
        self.signal3 = Signal(
            3, "3", Sentiment.NEUTRAL, self.date + datetime.timedelta(days=1)
        )
    ...
    def test_merge_signals(self):
        ...
        signal4 = Signal(
            4, "4", Sentiment.NEUTRAL, self.date - datetime.timedelta(days=1)
        )
        signal5 = Signal(
            5, "5", Sentiment.NEUTRAL, self.date + datetime.timedelta(days=1)
        )
        signal6 = Signal(
            6, "6", Sentiment.NEUTRAL, self.date + datetime.timedelta(days=2)
        )
        ...
        self.assertEqual(
            merged.signals,
            [signal4, self.signal, self.signal2, self.signal3, signal5, signal6],
        )
    ...
    def test_signals_since(self):
        ...
        self.assertEqual(
            ss.signals_since(self.date - datetime.timedelta(days=1)), signals
        )
