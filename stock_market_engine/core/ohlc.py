...
class OHLC:
	...
	def __eq__(self, other):
		...
		return [self.open, self.high, self.low, self.close] ==\
			   [other.open, other.high, other.low, other.close]

	def __repr__(self):
		return f"OHLC({self.end}, {self.open.values.iloc[-1]:.2f}, {self.high.values.iloc[-1]:.2f}, {self.low.values.iloc[-1]:.2f}, {self.close.values.iloc[-1]:.2f})"

	def to_json(self):
		return json.dumps({"dates" : self.dates.to_json(),
