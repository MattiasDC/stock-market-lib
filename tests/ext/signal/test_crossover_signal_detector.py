...
from stock_market.core import StockMarket, Ticker, SignalSequence, merge_signals, Sentiment
from stock_market.ext.indicator import Identity, ExponentialMovingAverage, register_indicator_factories
...
class TestCrossoverSignalDetector(unittest.TestCase):
	...
	def alternates_two_values(self, l):
		self.assertTrue(all_equal(islice(iter(l), 0, None, 2)))
		self.assertTrue(all_equal(islice(iter(l), 1, None, 2)))

		if len(l) > 1:
			self.assertNotEqual(l[0], l[1])
	...
	def validate_crossovers(self, crossover_signals, series, indicator_series):
		crossover_indices = series.dates.loc[series.dates.isin(map(lambda s: s.date, crossover_signals.signals))].index
		crossover_indices_and_before = crossover_indices.union(crossover_indices-1)
		differences = series.values.iloc[crossover_indices_and_before] -\
	...
	def test_detect(self):
		...
		sequence = SignalSequence()
		ema = ExponentialMovingAverage(20)
		bullish_detector = CrossoverSignalDetector(1,
												   "Bullish SPY Crossover EMA(20)",
												   spy,
												   Identity(),
												   ema,
												   Sentiment.BULLISH)
		bearish_detector = CrossoverSignalDetector(1,
												   "Bearish SPY Crossover EMA(20)",
												   spy,
												   Identity(),
												   ema,
												   Sentiment.BEARISH)
	...
	def test_json(self):
		spy = Ticker('SPY')
		ema = ExponentialMovingAverage(20)
		detector = CrossoverSignalDetector(1,
										   "Bullish SPY Crossover EMA(20)",
										   spy,
										   Identity(),
										   ema,
		...
		validate(instance=json.loads(json_str), schema=CrossoverSignalDetector.json_schema())
